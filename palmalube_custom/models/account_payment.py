...
class AccountPayment(models.Model):
    ...
    def _search(self, domain, offset=0, limit=None, order=None):
        ...
        y NO es manager, solo verá pagos donde él es el comercial_id.
        """
        ...
        if limited_group and limited_group in user.groups_id:
            ...
            if not manager_group or manager_group not in user.groups_id:
                # Agregar restricción: solo pagos donde el usuario es el comercial
                domain = domain + [('comercial_id', '=', user.id)]
